...
推送指向当前 commit 的 annotated `vX.Y.Z` tag 后，CI 会在完整 race、隔离、govulncheck 与 Trivy 门禁通过后发布 `linux/amd64`、`linux/arm64` GHCR manifest，并附带 BuildKit SBOM、max provenance、GitHub OIDC keyless provenance 与 `sandbox-image.json` digest 清单。平台部署只固定该不可变 manifest digest。
...
`deploy/deployment.yaml` 使用 `coderushoj/croj-sandbox:dev`，只面向本机 Kind；正式镜像由 annotated SemVer tag 构建并附加 GitHub OIDC provenance，生产级工作负载由 `croj-platform` Helm chart 以 digest 固定。`croj-judging-server` 只需要 EndpointSlice 的 `list` 权限，不需要访问 Pod 或 Kubernetes Endpoints API。
